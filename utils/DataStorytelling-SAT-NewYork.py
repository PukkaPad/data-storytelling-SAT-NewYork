...
# schools_heatmap.add_children(plugins.HeatMap([[row["lat"], row["lon"]] for name, row in base_data.iterrows()]))
...
mean_district_data["school_dist"] = mean_district_data["school_dist"].apply(lambda x: str(int(x))) #removes 0 from beginning
